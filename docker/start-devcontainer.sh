...
docker run -it --cap-add SYS_ADMIN \
    ...
    --mount type=bind,source=$(pwd)/workspace/legate-jax,target=/opt/workspace/legate-jax \
    ...
    --mount type=bind,source=$HOME/.cache/ccache/legate-jax,target=/ccache/legate-jax \
